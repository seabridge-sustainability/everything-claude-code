...
```bash
# Example for each tool Ã¢â‚¬â€ run each independently
START=$(date +%s)
...
DETAILS: Lint (3 warnings)
  biome check . output:
    src/utils.ts:42 Ã¢â‚¬â€ lint/complexity/noForEach: Prefer for...of
    src/api.ts:18 Ã¢â‚¬â€ lint/style/useConst: Use const instead of let
    src/api.ts:55 Ã¢â‚¬â€ lint/suspicious/noExplicitAny: Unexpected any
```
...
REGRESSIONS DETECTED
  Lint: 9 -> 6 (-3) Ã¢â‚¬â€ 12 new biome warnings introduced
    Most common: lint/complexity/noForEach (7 instances)
  Tests: 10 -> 9 (-1) Ã¢â‚¬â€ 2 test failures
