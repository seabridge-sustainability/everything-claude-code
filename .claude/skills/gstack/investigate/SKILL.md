...
Output: **"Root cause hypothesis: ..."** Ã¢â‚¬â€ a specific, testable claim about what is wrong and why.
...
- `git log` for prior fixes in the same area Ã¢â‚¬â€ **recurring bugs in the same files are an architectural smell**, not a coincidence

**External pattern search:** If the bug doesn't match a known pattern above, WebSearch for:
- "{framework} {generic error type}" Ã¢â‚¬â€ **sanitize first:** strip hostnames, IPs, file paths, SQL, customer data. Search the error category, not the raw message.
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use AskUserQuestion:
   ...
   A) Continue investigating Ã¢â‚¬â€ I have a new hypothesis: [describe]
   B) Escalate for human review Ã¢â‚¬â€ this needs someone who knows the system
   C) Add logging and wait Ã¢â‚¬â€ instrument the area and catch it next time
   ```

**Red flags** Ã¢â‚¬â€ if you see any of these, slow down:
- "Quick fix for now" Ã¢â‚¬â€ there is no "for now." Fix it right or escalate.
- Proposing a fix before tracing data flow Ã¢â‚¬â€ you're guessing.
- Each fix reveals a new problem elsewhere Ã¢â‚¬â€ wrong layer, not wrong code.
...
5. **If the fix touches >5 files:** Use AskUserQuestion to flag the blast radius:
   ...
   A) Proceed Ã¢â‚¬â€ the root cause genuinely spans these files
   B) Split Ã¢â‚¬â€ fix the critical path now, defer the rest
   C) Rethink Ã¢â‚¬â€ maybe there's a more targeted approach
   ```
...
- **3+ failed fix attempts Ã¢â€ â€™ STOP and question the architecture.** Wrong architecture, not failed hypothesis.
- **Never apply a fix you cannot verify.** If you can't reproduce and confirm, don't ship it.
- **Never say "this should fix it."** Verify and prove it. Run the tests.
- **If fix touches >5 files Ã¢â€ â€™ AskUserQuestion** about blast radius before proceeding.
- **Completion status:**
  - DONE Ã¢â‚¬â€ root cause found, fix applied, regression test written, all tests pass
  - DONE_WITH_CONCERNS Ã¢â‚¬â€ fixed but cannot fully verify (e.g., intermittent bug, requires staging)
  - BLOCKED Ã¢â‚¬â€ root cause unclear after investigation, escalated
