...
```text
merge: PR #529 Ã¢â‚¬â€ feat(skills): add documentation-lookup, bun-runtime, nextjs-turbopack; feat(agents): add rust-reviewer
```
...
No specific test framework detected Ã¢â‚¬â€ use the repository's existing test patterns.
...
fix: address PR review Ã¢â‚¬â€ skill template (When to use, How it works, Examples), bun.lock, next build note, rust-reviewer CI note, doc-lookup privacy/uncertainty
```
