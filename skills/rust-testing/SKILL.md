...
1. **Identify target code** Ã¢â‚¬â€ Find the function, trait, or module to test
2. **Write a test** Ã¢â‚¬â€ Use `#[test]` in a `#[cfg(test)]` module, rstest for parameterized tests, or proptest for property-based tests
3. **Mock dependencies** Ã¢â‚¬â€ Use mockall to isolate the unit under test
...
Ã¢â€â€š   Ã¢â€Å“Ã¢â€â‚¬Ã¢â€â‚¬ api_test.rs     # Each file is a separate test binary
...
- Keep tests independent Ã¢â‚¬â€ no shared mutable state

**DON'T:**
- Use `#[should_panic]` when you can test `Result::is_err()` instead
- Mock everything Ã¢â‚¬â€ prefer integration tests when feasible
- Ignore flaky tests Ã¢â‚¬â€ fix or quarantine them
- Use `sleep()` in tests Ã¢â‚¬â€ use channels, barriers, or `tokio::time::pause()`
