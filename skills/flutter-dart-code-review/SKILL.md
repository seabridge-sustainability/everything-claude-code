...
- [ ] `pubspec.yaml` is clean Ã¢â‚¬â€ no unused dependencies, versions pinned appropriately
- [ ] `analysis_options.yaml` includes a strict lint set with strict analyzer settings enabled
- [ ] No `print()` statements in production code Ã¢â‚¬â€ use `dart:developer` `log()` or a logging package
...
- [ ] **Implicit dynamic**: Missing type annotations leading to `dynamic` Ã¢â‚¬â€ enable `strict-casts`, `strict-inference`, `strict-raw-types`
...
- [ ] **Unused `async`**: Functions marked `async` that never `await` Ã¢â‚¬â€ unnecessary overhead
...
### Const usage:
- [ ] `const` constructors used wherever possible Ã¢â‚¬â€ prevents unnecessary rebuilds
...
- [ ] `GlobalKey` used sparingly Ã¢â‚¬â€ only when accessing state across the tree is truly needed
- [ ] `UniqueKey` avoided in `build()` Ã¢â‚¬â€ it forces rebuild every frame
- [ ] `ObjectKey` used when identity is based on a data object rather than a single value

### Theming & design system:
- [ ] Colors come from `Theme.of(context).colorScheme` Ã¢â‚¬â€ no hardcoded `Colors.red` or hex values
- [ ] Text styles come from `Theme.of(context).textTheme` Ã¢â‚¬â€ no inline `TextStyle` with raw font sizes
- [ ] Dark mode compatibility verified Ã¢â‚¬â€ no assumptions about light background
...
### Architecture:
- [ ] Business logic lives outside the widget layer Ã¢â‚¬â€ in a state management component (BLoC, Notifier, Controller, Store, ViewModel, etc.)
- [ ] State managers receive dependencies via injection, not by constructing them internally
- [ ] A service or repository layer abstracts data sources Ã¢â‚¬â€ widgets and state managers should not call APIs or databases directly
- [ ] State managers have a single responsibility Ã¢â‚¬â€ no "god" managers handling unrelated concerns
- [ ] Cross-component dependencies follow the solution's conventions:
  - In **Riverpod**: providers depending on providers via `ref.watch` is expected Ã¢â‚¬â€ flag only circular or overly tangled chains
  - In **BLoC**: blocs should not directly depend on other blocs Ã¢â‚¬â€ prefer shared repositories or presentation-layer coordination
...
- [ ] State objects are immutable Ã¢â‚¬â€ new instances created via `copyWith()` or constructors, never mutated in-place
- [ ] State classes implement `==` and `hashCode` properly (all fields included in comparison)
- [ ] Mechanism is consistent across the project Ã¢â‚¬â€ manual override, `Equatable`, `freezed`, Dart records, or other
...
- [ ] State is only mutated through the solution's reactive API (`@action` in MobX, `.value` on signals, `.obs` in GetX) Ã¢â‚¬â€ direct field mutation bypasses change tracking
...
### State shape design:
- [ ] Mutually exclusive states use sealed types, union variants, or the solution's built-in async state type (e.g. Riverpod's `AsyncValue`) Ã¢â‚¬â€ not boolean flags (`isLoading`, `isError`, `hasData`)
- [ ] Every async operation models loading, success, and error as distinct states
- [ ] All state variants are handled exhaustively in UI Ã¢â‚¬â€ no silently ignored cases
- [ ] Error states carry error information for display; loading states don't carry stale data
- [ ] Nullable data is not used as a loading indicator Ã¢â‚¬â€ states are explicit
...
- [ ] Selectors used to rebuild only when specific fields change Ã¢â‚¬â€ not on every state emission
...
- [ ] `BuildContext` not used after `await` without checking `context.mounted` (Flutter 3.7+) Ã¢â‚¬â€ stale context causes crashes
...
- [ ] Shared state is lifted only as high as needed Ã¢â‚¬â€ not over-globalized
...
- [ ] `setState()` not called at root widget level Ã¢â‚¬â€ localize state changes
...
- [ ] No sorting, filtering, or mapping large collections in `build()` Ã¢â‚¬â€ compute in state management layer
...
### Other:
- [ ] `Opacity` widget avoided in animations Ã¢â‚¬â€ use `AnimatedOpacity` or `FadeTransition`
- [ ] Clipping avoided in animations Ã¢â‚¬â€ pre-clip images
- [ ] `operator ==` not overridden on widgets Ã¢â‚¬â€ use `const` constructors instead
...
- [ ] No flaky tests depending on timing Ã¢â‚¬â€ use `pumpAndSettle` or explicit `pump(Duration)`
...
- [ ] No no-op `onPressed` callbacks Ã¢â‚¬â€ every button does something or is disabled
...
- [ ] Text doesn't overflow on small screens Ã¢â‚¬â€ use `Flexible`, `Expanded`, `FittedBox`
...
### API key handling:
- [ ] API keys NOT hardcoded in Dart source Ã¢â‚¬â€ use `--dart-define`, `.env` files excluded from VCS, or compile-time configuration
- [ ] Secrets not committed to git Ã¢â‚¬â€ check `.gitignore`
...
- [ ] Check last publish date Ã¢â‚¬â€ stale packages (>1 year) are a risk
...
### Version constraints:
- [ ] Use caret syntax (`^1.2.3`) for dependencies Ã¢â‚¬â€ allows compatible updates
- [ ] Pin exact versions only when absolutely necessary
- [ ] Run `flutter pub outdated` regularly to track stale dependencies
- [ ] No dependency overrides in production `pubspec.yaml` Ã¢â‚¬â€ only for temporary fixes with a comment/issue link
- [ ] Minimize transitive dependency count Ã¢â‚¬â€ each dependency is an attack surface

### Monorepo-specific (melos/workspace):
- [ ] Internal packages import only from public API Ã¢â‚¬â€ no `package:other/src/internal.dart` (breaks Dart package encapsulation)
...
- [ ] One routing approach used consistently Ã¢â‚¬â€ no mixing imperative `Navigator.push` with a declarative router
- [ ] Route arguments are typed Ã¢â‚¬â€ no `Map<String, dynamic>` or `Object?` casting
- [ ] Route paths defined as constants, enums, or generated Ã¢â‚¬â€ no magic strings scattered in code
- [ ] Auth guards/redirects centralized Ã¢â‚¬â€ not duplicated across individual screens
- [ ] Deep links configured for both Android and iOS
- [ ] Deep link URLs validated and sanitized before navigation
- [ ] Navigation state is testable Ã¢â‚¬â€ route changes can be verified in tests
...
- [ ] Raw exceptions (network, parsing) are mapped to user-friendly, localized messages before reaching the UI Ã¢â‚¬â€ never show raw exception strings to users
...
### Content:
- [ ] All user-visible strings use the localization system Ã¢â‚¬â€ no hardcoded strings in widgets
...
- [ ] No string concatenation for localized text Ã¢â‚¬â€ use parameterized messages
...
- [ ] Dependencies provided externally via constructor, DI framework, or provider graph Ã¢â‚¬â€ not created internally
...
- [ ] `prefer_const_constructors` Ã¢â‚¬â€ performance in widget trees
- [ ] `avoid_print` Ã¢â‚¬â€ use proper logging
- [ ] `unawaited_futures` Ã¢â‚¬â€ prevent fire-and-forget async bugs
- [ ] `prefer_final_locals` Ã¢â‚¬â€ immutability at variable level
- [ ] `always_declare_return_types` Ã¢â‚¬â€ explicit contracts
- [ ] `avoid_catches_without_on_clauses` Ã¢â‚¬â€ specific error handling
- [ ] `always_use_package_imports` Ã¢â‚¬â€ consistent import style
