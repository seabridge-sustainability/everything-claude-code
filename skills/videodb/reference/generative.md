...
Returns an `Image` object with `.id`, `.name`, and `.collection_id`. The `.url` property may be `None` for generated images Ã¢â‚¬â€ always use `image.generate_url()` to get a reliable signed download URL.
