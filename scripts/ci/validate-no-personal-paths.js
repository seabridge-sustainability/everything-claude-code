...
const EXEMPT_PREFIXES = [
  'docs/fixes/',
  // SeaBridge fork: gitignored local scan evidence quotes machine paths verbatim.
  'docs/reports/',
];
...
// SeaBridge fork: canonical protocol pointers in this workspace are absolute
// paths under the operator's home directory, so that username is not a leak here.
const ALLOWED_WORKSPACE_USERNAMES = new Set([
  'adelm',
]);
...
function findLeaks(content) {
  ...
  for (const pattern of [POSIX_USER_RE, WIN_USER_RE]) {
    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (!PLACEHOLDER_USERNAMES.has(username) && !ALLOWED_WORKSPACE_USERNAMES.has(username)) {
        leaks.push(match[0]);
      }
    }
  }
