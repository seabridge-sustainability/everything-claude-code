...
OpenCode must load the centralized `AGENTS_SYSTEM.md` / `SEABRIDGE_CODING_AGENT_SYSTEM.md` contract first, then repo-local `AGENTS.md`, `CLAUDE.md`, and `AGENTS_SYSTEM.md` overrides.

## Execution Cheatsheet

- Load first: `SEABRIDGE_CODING_AGENT_SYSTEM.md`, `AGENTS_SYSTEM.md`, then the instruction files configured by `opencode.jsonc`.
...
- Skills: read canonical `skills/sea-*/SKILL.md` bodies or wrappers configured by the harness; wrappers only point to canonical behavior.
...
- Unsupported native commands: if a Claude/Codex/Gemini command has no OpenCode equivalent, use the local file/tool workflow and document the unsupported step.
...
- OpenCode should reference `AGENTS.md` / `CLAUDE.md` from its instruction config and read matching `SKILL.md` files directly.
...
- `/goal <task>` or the local equivalent triggers autonomous persistent execution.
- `/goal` and auto-loop are the same mode; `/goal` is the command pattern, auto-loop is the behavior.
- For long-running or non-trivial work, OpenCode must define the objective, DoD, phases, validation plan, stuck-task strategy, and completion evidence before claiming success.
...
OpenCode follows the same self-verification loop: plan before edits, write/update tests when practical, prove red before green when practical, run focused checks, broaden checks when risk warrants it, document skipped tests, and never claim completion from code changes alone.
