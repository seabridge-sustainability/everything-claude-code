...
- Load first: `SEABRIDGE_CODING_AGENT_SYSTEM.md`, `AGENTS_SYSTEM.md`, then repo-local `GEMINI.md`/`AGENTS.md`.
...
- Skills: activate native Gemini skills when configured or read canonical `skills/sea-*/SKILL.md` bodies; wrappers only point to canonical behavior.
...
- Unsupported native commands: if a Claude/Codex/OpenCode command has no Gemini equivalent, use the local file/tool workflow and document the unsupported step.
...
- `goal:` or `/goal <task>` triggers autonomous persistent execution.
- `/goal` and auto-loop are the same mode; `/goal` is the command pattern, auto-loop is the behavior.
- For long-running or non-trivial work, Gemini must define the objective, DoD, phases, validation plan, stuck-task strategy, and completion evidence before claiming success.
