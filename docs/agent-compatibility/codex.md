...
Codex must load the centralized `AGENTS_SYSTEM.md` / `SEABRIDGE_CODING_AGENT_SYSTEM.md` contract first, then repo-local `AGENTS.md`, `CLAUDE.md`, and `AGENTS_SYSTEM.md` overrides.

## Execution Cheatsheet

- Load first: `SEABRIDGE_CODING_AGENT_SYSTEM.md`, `AGENTS_SYSTEM.md`, then the repo-local `AGENTS.md`/sibling instruction files Codex sees.
...
- Skills: read canonical `skills/sea-*/SKILL.md` bodies or `.agents/skills/sea-*` wrappers; wrappers only point to canonical behavior.
...
- Unsupported native commands: if a Claude/Gemini/OpenCode command has no Codex equivalent, use the local file/tool workflow and document the unsupported step.
...
- Codex-supported wrappers: `C:\Users\adelm\SeaBridgeAI\everything-claude-code\.agents\skills\sea-*\SKILL.md`
- Prefer wrappers where available; fall back to canonical skill bodies.
...
- `$goal <task>` or the `goal-default` skill triggers autonomous persistent execution.
- `/goal` and auto-loop are the same mode; `/goal` is the command pattern, auto-loop is the behavior.
- For long-running or non-trivial work, Codex must define the objective, DoD, phases, validation plan, stuck-task strategy, and completion evidence before claiming success.
...
Codex follows the same self-verification loop: plan before edits, write/update tests when practical, prove red before green when practical, run focused checks, broaden checks when risk warrants it, document skipped tests, and never claim completion from code changes alone.
