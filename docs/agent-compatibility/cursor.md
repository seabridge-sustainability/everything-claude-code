...
- Cursor should read matching `SKILL.md` files directly from rules or workspace context.
...
Cursor follows the same self-verification loop: plan before edits, write/update tests when practical, prove red before green when practical, run focused checks, broaden checks when risk warrants it, document skipped tests, and never claim completion from code changes alone.
