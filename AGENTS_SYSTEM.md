...
> **Status (2026-09-24): reference document, not a startup read.** No harness
> auto-loads this file, and agents do not need to read it before working. The
> operative rules reach every agent through each repo's `AGENTS.md`: the
> canonical safety block (`protocols/SAFETY_AUTHORIZATION_RULE.md`), the goal
> block (`scripts/sync-goal-protocol.ps1`), and repo facts; `CLAUDE.md` imports
> `AGENTS.md`. Where this file disagrees with those, they win. Design record:
...
follow its routing. State it when no skill was needed. Exception: mandatory
named triggers are not waived by the one-skill default — cross-repo changes
always use `sea-cross-repo-handoff`, and done/fixed/production-ready claims
always use `sea-verification-before-completion` (see Mandatory Skill Triggers
in `SEABRIDGE_CODING_AGENT_SYSTEM.md`). New skills become available through
filesystem inspection, not through product-repo skill matrices.
...
This block is marker-synced from `protocols/SAFETY_AUTHORIZATION_RULE.md`; edit
only that file, then run `scripts/sync-safety-rule.ps1`.
